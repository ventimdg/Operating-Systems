...
/*this will be the list used for our strict priority scheduler*/
static struct list priority_ready_list[64];
...
static struct list timer_sleep_list; // List of all threads that called timer_sleep

//Function so we can get the sleep list in the timer file
...
void thread_init(void) {
  ...
  //Initializing the list that tracks all threads that are sleeping because of calls to timer_sleep
...
//Moves a thread on the ready list into the proper place if its effective priority has been updated
void move_thread_priority_list(struct thread* t) {
  list_remove(&t->priority_ready_list_elem);
  list_push_back(&priority_ready_list[t->effective_priority], &t->priority_ready_list_elem);
}
...
void thread_tick(void) {
  ...
  int64_t tick_since_OS = timer_ticks();

  struct list_elem* e;
  int priority = -1;
  for (e = list_begin(&timer_sleep_list); e != list_end(&timer_sleep_list);) {
    struct thread* curr_thread = list_entry(e, struct thread, sleep_elem);
    if (curr_thread->target_tick_wakeup == tick_since_OS) {
...
void thread_set_priority(int new_priority) {
  ...
  if (t->effective_priority == t->priority) {
    bool lowered = new_priority < t->effective_priority;
...
static void init_thread(struct thread* t, const char* name, int priority) {
  ...
  t->thread_donate_to = NULL;
  t->lock_donate_to = NULL;
  list_init(&t->locks);
  t->pcb = NULL;
...
static struct thread* thread_schedule_prio(void) {
  for (int i = 63; i >= 0; i--) {
    if (!list_empty(&priority_ready_list[i])) {
      return (list_entry(list_pop_front(&priority_ready_list[i]), struct thread,
                         priority_ready_list_elem));
    }
  }
  return idle_thread;
