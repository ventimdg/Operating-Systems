...
bool setup_thread(void (**eip)(void), void** esp, metadata_t* data);
...
void userprog_init(void) {
  ...
  if (success) {
    ...
    list_init(&t->pcb->initialized_locks);
    t->pcb->next_lock = 1;
    list_init(&t->pcb->initialized_semaphores);
    t->pcb->next_semaphore = 1;
    lock_init(&t->pcb->sem_lock);
    lock_init(&t->pcb->lock_lock);
    lock_init(&t->pcb->general_lock);

    t->pcb->thread_count = 1;

    list_init(&t->pcb->thread_list);
    list_init(&t->pcb->removed);

    struct tid_elem* main_elem = (struct tid_elem*)malloc(sizeof(struct tid_elem));
    t->join_opt = main_elem;
    main_elem->tid = t->tid;
    sema_init(&main_elem->join, 0);
    main_elem->been_waited_on = false;
    main_elem->has_terminated = false;

    main_elem->pcb = t->pcb;

    lock_acquire(&t->pcb->general_lock);
    list_push_back(&t->pcb->thread_list, &main_elem->elem);
    lock_release(&t->pcb->general_lock);

    t->pcb->has_exited = false;
    t->pcb->main_exited = false;
    t->pcb->exception = false;
    t->pcb->printed = false;
    t->pcb->final_exit = -4;
  }
...
void process_exit(int exit_code) {
  ...
  if (cur != cur->pcb->main_thread) {
    ...
    if (exit_code < 0) {
      cur->pcb->exception = true;
    }
    pthread_exit();
  }

  if (!cur->pcb->main_exited) {
    pthread_exit_main(exit_code);
  }
  ...
  } else if (cur->pcb->exception) {
    printf("%s: exit(%d)\n", thread_current()->pcb->process_name, -1);
  } else if (cur->pcb->final_exit != -4) {
    printf("%s: exit(%d)\n", thread_current()->pcb->process_name, cur->pcb->final_exit);
  } else if (exit_code == -3) {
  ...
  for (e = list_begin(&cur->pcb->thread_list); e != list_end(&cur->pcb->thread_list);) {
    struct tid_elem* thread_join = list_entry(e, struct tid_elem, elem);
    e = list_next(e);
    free(thread_join);
  }

  for (e = list_begin(&cur->pcb->removed); e != list_end(&cur->pcb->removed);) {
    struct tid_elem* thread_join = list_entry(e, struct tid_elem, elem);
    e = list_next(e);
    free(thread_join);
  }

  for (e = list_begin(&cur->pcb->initialized_locks); e != list_end(&cur->pcb->initialized_locks);) {
    struct userspace_lock* thread_join = list_entry(e, struct userspace_lock, elem);
    e = list_next(e);
    free(thread_join);
  }

  for (e = list_begin(&cur->pcb->initialized_semaphores);
       e != list_end(&cur->pcb->initialized_semaphores);) {
    struct userspace_semaphore* thread_join = list_entry(e, struct userspace_semaphore, elem);
    e = list_next(e);
    free(thread_join);
  }
...
static bool setup_stack(void** esp) {
  ...
  if (kpage != NULL) {
    ...
    if (success) {
      *esp = PHYS_BASE;
      t->user_stack = (void*)((uint8_t*)*esp - PGSIZE);
    } else {
      palloc_free_page(kpage);
    }
  }
...
bool setup_thread(void (**eip)(void), void** esp, metadata_t* data) {
  ...
  struct tid_elem* thread_elem = (struct tid_elem*)malloc(sizeof(struct tid_elem));
  t->join_opt = thread_elem;
  thread_elem->tid = t->tid;
  sema_init(&thread_elem->join, 0);
  thread_elem->been_waited_on = false;
  ...
  bool success = install_page(uaddr, kpage, true);

  while (!success) {
    uaddr -= PGSIZE;
    numpages++;
    if (numpages > MAX_STACK_PAGES) {
      data->load_check = false;
      sema_up(&(t->shared->load));
      free(thread_elem);
      return false;
    }
  ...
  t->user_stack = (void*)uaddr;

  *esp = uaddr + PGSIZE;

  *esp -= (int)(*esp - sizeof(data->arg) - sizeof(data->tf)) % 16;

  *esp -= sizeof(data->arg);
  memcpy(*esp, &data->arg, sizeof(data->arg));

  *esp -= sizeof(data->tf);
  memcpy(*esp, &data->tf, sizeof(data->tf));

  *esp -= 4;
  *(uint32_t*)*esp = 0;

  *eip = (void (*)(void))data->sf;

  data->load_check = true;

  lock_acquire(&t->pcb->general_lock);
  t->pcb->thread_count++;
  lock_release(&t->pcb->general_lock);

  thread_elem->has_terminated = false;
  thread_elem->pcb = t->pcb;
...
tid_t pthread_execute(stub_fun sf, pthread_fun tf, void* arg) {
  ...
  /* Create a new thread to execute FILE_NAME. */
  lock_acquire(&t->pcb->general_lock);
  int name = t->pcb->thread_count;
  lock_release(&t->pcb->general_lock);

  tid = thread_create("child", PRI_DEFAULT, start_pthread, (void*)data);
  //call down on child's load semaphore to wait till it's attempted to load
  sema_down(&(data->load));
...
static void start_pthread(void* exec_ UNUSED) {
  ...
  //cast passed void pointer to a metadata struct
  metadata_t* data = (metadata_t*)exec_;
  ...
  asm volatile("fsave %0" : "=m"(if_.fpu));
  process_activate();
  success = setup_thread(&if_.eip, &if_.esp, data);
...
tid_t pthread_join(tid_t tid) {
  struct thread* t = thread_current();
  struct list_elem* e;
  for (e = list_begin(&t->pcb->thread_list); e != list_end(&t->pcb->thread_list);
       e = list_next(e)) {
    struct tid_elem* thread_join = list_entry(e, struct tid_elem, elem);
    if (thread_join->pcb->main_thread->tid == t->pcb->main_thread->tid) {
      if (tid == thread_join->tid) {
        if (!thread_join->been_waited_on) {
          if (!thread_join->has_terminated) {
            thread_join->been_waited_on = true;

            sema_down(&thread_join->join);

            lock_acquire(&t->pcb->general_lock);
            list_remove(e);
            list_push_back(&t->pcb->removed, e);
            lock_release(&t->pcb->general_lock);
...
void pthread_exit(void) {
  ...
  sema_up(&cur->join_opt->join);
  cur->join_opt->has_terminated = true;
  cur->pcb->thread_count--;

  palloc_free_page(pagedir_get_page(t->pcb->pagedir, t->user_stack));
...
void pthread_exit_main(int exit_code) {
  ...
  for (e = list_begin(&t->pcb->thread_list); e != list_end(&t->pcb->thread_list);
       e = list_next(e)) {
    struct tid_elem* thread_join = list_entry(e, struct tid_elem, elem);
    if (thread_join->tid == t->tid) {
      thread_join->has_terminated = true;
      sema_up(&thread_join->join);
      thread_join->pcb->thread_count--;
      break;
    }
  }
  ...
  for (e = list_begin(&t->pcb->thread_list); e != list_end(&t->pcb->thread_list);) {
    ...
    e = list_next(e);
    if (thread_join->tid != t->tid) {
      pthread_join(thread_join->tid);
    }
  }

  t->pcb->has_exited = true;
  if (t->pcb->exception) {
    process_exit(-2);
  } else {
