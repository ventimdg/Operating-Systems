...
file_descriptor_t* add_fd(file_descriptor_list_t* fdlist, int fd, void* curfile, const char* name,
                          bool is_dir) {
  /* TODO */
  if (fdlist == NULL) {
    return NULL;
  ...
  if (is_dir) {
    new_elem->directory = (struct dir*)curfile;
    new_elem->is_dir = is_dir;
  } else {
    new_elem->is_dir = is_dir;
    new_elem->open_file = (struct file*)curfile;
  }
...
void process_exit(int exit_code) {
  ...
  struct inode* old_cwd = inode_open(cur->pcb->CWD);
...
bool load(const char* file_name, void (**eip)(void), void** esp) {
  ...
  bool is_dir;
  file = (struct file*)filesys_open(argv[0], &is_dir);
  if (file == NULL) {
