...
void fsutil_cat(char** argv) {
  ...
  bool is_dir;
  file = (struct file*)filesys_open(file_name, &is_dir);
  if (file == NULL)
...
void fsutil_extract(char** argv UNUSED) {
  ...
  for (;;) {
    ...
    cache_read(src, sector, header, sector);
    sector++;
    ...
    if (type == USTAR_EOF) {
      ...
      bool is_dir;
      dst = (struct file*)filesys_open(file_name, &is_dir);
      if (dst == NULL)
        PANIC("%s: open failed", file_name);
      ...
      while (size > 0) {
        ...
        cache_read(src, sector, data, sector);
        sector++;
...
void fsutil_append(char** argv) {
  ...
  /* Open source file. */
  bool is_dir;
  src = (struct file*)filesys_open(file_name, &is_dir);
  if (src == NULL)
    PANIC("%s: open failed", file_name);
