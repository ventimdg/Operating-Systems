...
void serve_file(int fd, char* path) {
  ...
  stat(path, &buf);
  char size[33];
  snprintf(size, 33, "%lu", (unsigned long) buf.st_size);

  http_start_response(fd, 200);
  http_send_header(fd, "Content-Type", http_get_mime_type(path));
  http_send_header(fd, "Content-Length", size); // TODO: change this line too
  http_end_headers(fd);

  int newfd = open(path, O_RDONLY);
  char read_buf[500];
  ssize_t numread;
  while ((numread = read(newfd, read_buf, sizeof(read_buf))) > 0){
    write(fd, read_buf, (size_t) numread);
  }
  close(newfd);
  /* PART 2 END */
}
...
void serve_directory(int fd, char* path) {
  ...
  if (directory != NULL) {
    ...
    while(entry != NULL){
      int length = strlen(path) + 2 * strlen(entry->d_name) + 100;
      char formatted[length];
      http_format_href(formatted, path, entry->d_name);
...
void* to_proxy(void* args) {
  int* newargs = (int*) args;
  int fd = newargs[0];
  int target_fd = newargs[1];;

  char buffer[1000];
  ssize_t numread;

  while ((numread = read(fd, buffer, 1000)) >= 0) {
    write(target_fd, buffer, numread);
  }
...
void* from_proxy(void* args) {
  ...
  while ((numread = read(target_fd, buffer, 1000)) >= 0) {
...
void handle_proxy_request(int fd) {
  ...
  */
  ...
  pthread_create(&t1, NULL, to_proxy, (void *) args);
  pthread_create(&t2, NULL, from_proxy, (void *) args);
...
void* thread_wait(void* void_request_handler) {
  pthread_mutex_lock(&work_queue.mutex);
  pthread_cond_wait(&work_queue.condvar, &work_queue.mutex);
  pthread_mutex_unlock(&work_queue.mutex);
  handle_clients(void_request_handler);
}
...
void init_thread_pool(int num_threads, void (*request_handler)(int)) {
  ...
  for (int i = 0; i < num_threads; i++){
    pthread_t new;
    pthread_create(&new, NULL, thread_wait, (void*) request_handler);
  }
  /* PART 7 END */
}
#endif
...
void serve_forever(int* socket_number, void (*request_handler)(int)) {
  ...
  *socket_number = socket(PF_INET, SOCK_STREAM, 0);
  ...
  /* PART 1 BEGIN */
  int bound = bind(*socket_number, (struct sockaddr*) &server_address, sizeof(server_address));
  if (bound == -1) {
  ...
  while (1) {
    ...
    /* PART 6 BEGIN */
    pthread_t t;
    if (pthread_create(&t, NULL, (void*) request_handler, (void*) client_socket_number) != 0) {
      close(client_socket_number);
    }
    pthread_detach(t);
    /* PART 6 END */
